...
import type { Song } from '../types'; // Corrected path
import SongCard from '../components/SongCard'; // Corrected path
...
const SearchResults = () => {
  ...
  useEffect(() => {
    ...
    const fetchSearchResults = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`${API_BASE_URL}/api/search-music?query=${encodeURIComponent(query)}`, {
          credentials: 'include', // IMPORTANT: Send cookies for authenticated requests
        });
        if (!response.ok) {
